...
public class StudentService {
    ...
    @Autowired
    private DomesticViolenceRepository domesticViolenceRepository;
    ...
    public StudentWrapper findStudent(Integer studentId) {
        ...
        try {
            ...
            if (student.isPresent()) {
                ...
                MentalHealth mentalHealth = mentalHealthRepository.findByStudentId(studentId);
                if (mentalHealth == null) {
                    mentalHealth = new MentalHealth();
                }
                studentWrapper.setMentalHealth(mentalHealth);

                DomesticViolence domesticViolence = domesticViolenceRepository.findByStudentId(studentId);
                if (domesticViolence == null) {
                    domesticViolence = new DomesticViolence();
                }
                studentWrapper.setDomesticViolence(domesticViolence);
    ...
    public Response createNewStudent(StudentWrapper studentWrapper) {
        ...
        try{
            ...
            MentalHealth mentalHealth = studentWrapper.getMentalHealth();
            mentalHealth.setStudentId(id);
            mentalHealth = mentalHealthRepository.save(mentalHealth);
            studentWrapper.setMentalHealth(mentalHealth);

            DomesticViolence domesticViolence = studentWrapper.getDomesticViolence();
            domesticViolence.setStudentId(id);
            domesticViolence = domesticViolenceRepository.save(domesticViolence);
            studentWrapper.setDomesticViolence(domesticViolence);
